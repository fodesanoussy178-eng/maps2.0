(function(root) {
  ...
  function candidat(lieu) {
    ...
    return {
      ...
      distanceM: Number.isFinite(Number(l.rankDistance)) ? Math.round(Number(l.rankDistance)) : null,
  ...
  const CLES_UTILES = Object.freeze([
    "amenity",
    "social_facility",
    "social_facility:for",
    "office",
  ...
  function tagsUtiles(lieu) {
    ...
    const out = {};
    CLES_UTILES.forEach((k) => {
      if (tags[k]) out[k] = String(tags[k]);
    });
    return out;
  }
  function contexte(etat) {
    const e = etat || {};
    const candidats = (Array.isArray(e.candidatePlaces) ? e.candidatePlaces : []).map(candidat);
    return {
  ...
  const REGLES = Object.freeze([
    "Ne jamais inventer une structure, une adresse, un horaire ou un t\xE9l\xE9phone.",
    "Ne retenir que des identifiants pr\xE9sents dans allowedPlaceIds.",
    "Ne jamais affirmer qu\u2019une structure fournit un service que les donn\xE9es ne montrent pas.",
    "Ne retenir que des cat\xE9gories pr\xE9sentes dans allowedCategories.",
    "Pr\xE9f\xE9rer deux structures s\xFBres \xE0 dix approximatives."
  ]);
  ...
  function valider(reponse, ctx) {
    ...
    const primaire = r.primaryNeed && garderCategorie(r.primaryNeed) ? String(r.primaryNeed) : null;
    const secondaires = (Array.isArray(r.secondaryNeeds) ? r.secondaryNeeds : []).filter(garderCategorie).map(String).filter((id) => id !== primaire);
    const ordre = (Array.isArray(r.rankedPlaceIds) ? r.rankedPlaceIds : []).map(String).filter(garderId);
    const ecartes = (Array.isArray(r.rejectedPlaceIds) ? r.rejectedPlaceIds : []).map(String).filter((id) => permis.has(id));
    const explications = {};
  ...
  function appliquer(resultats, valide) {
    ...
    return liste.filter((l) => !ecartes.has(String(l.id))).slice().sort((a, b) => {
