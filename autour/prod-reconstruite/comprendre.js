(function(root) {
  ...
  function montant(t) {
    const chiffre = /(?:moins de|max(?:imum)?|jusqu a|sous|pas plus de|-\s*de)?\s*(\d{1,3})\s*(?:e\b|eur\b|euros?|€|balles?)/.exec(t);
  ...
  function nettoyer(t, consommes) {
    let reste = t;
    (consommes || []).forEach((m) => {
      reste = reste.split(sansAccents(m)).join(" ");
    });
  ...
  function trouver(table, t) {
    ...
    Object.entries(table).forEach(([id, def]) => {
      const mot = (def.mots || []).slice().sort((a, b) => b.length - a.length).find((m) => contient(t, m));
  ...
  function analyser(phrase, options) {
    ...
    trouver(ONTOLOGIE, t).forEach(({ id, def, mot }) => {
      ...
      if (id === "gratuit") {
        intention.budget.gratuit = true;
        return;
      }
      if (id === "pas_cher") {
        intention.budget.pasCher = true;
        return;
      }
      const poids = 1;
      ...
      (def.implique || []).forEach((autre) => {
        if (!intention.ambiance.some((a) => a.id === autre))
          intention.ambiance.push({ id: autre, poids: 0.6, implicite: true });
      });
      (def.dur ? intention.contraintes : intention.preferences).push({ type: "signal", id, poids });
    ...
    if (typeof o.cuisineDe === "function") {
      ...
      if (c) {
        intention.cuisine = c;
        ajouterChip("cuisine:" + c, "cuisine", o.libelleCuisine ? o.libelleCuisine(c) : c);
      }
    }
    if (typeof o.categorieDe === "function") {
      ...
      if (cat) {
        cats.add(cat);
        ajouterChip("cat:" + cat, "categorie", o.libelleCategorie && o.libelleCategorie(cat) || cat);
      }
    }
    const n = montant(t);
    if (n != null) {
    ...
    if (intention.horaire.creneau)
      ajouterChip(
        "creneau:" + intention.horaire.creneau,
        "horaire",
        { maintenant: "Maintenant", soir: "Ce soir", weekend: "Ce week-end", avenir: "\xC0 venir" }[intention.horaire.creneau]
      );
    if (intention.horaire.apres != null) {
      intention.contraintes.push({ type: "ouvertApres", minutes: intention.horaire.apres });
      ajouterChip("horaire:apres", "contrainte", "Apr\xE8s " + Math.floor(intention.horaire.apres / 60) + "h");
    ...
    if (PROCHE.test(t)) {
      ...
      ajouterChip("proche", "ambiance", "Tout pr\xE8s");
    }
  ...
  function estVocabulaire(fragment, options) {
    ...
    if (!t) return true;
    if (HEURE_SEULE.test(t)) return true;
  ...
  function estExploitable(intention) {
    ...
    return !!(intention.categories.length || intention.intentions.length || intention.ambiance.length || intention.cuisine || intention.budget.max != null || intention.budget.gratuit || intention.horaire.creneau || intention.contraintes.length);
  }
  function sansChip(intention, id) {
    ...
    if (type === "horaire") {
      copie.horaire.tard = false;
      copie.horaire.apres = null;
  ...
  function noterReste(reste) {
    ...
    if (cles.length > JOURNAL_MAX) {
      ...
      try {
        root.localStorage.setItem(CLE_JOURNAL, JSON.stringify(reduit));
      } catch (e) {
      }
      return true;
    }
    try {
      root.localStorage.setItem(CLE_JOURNAL, JSON.stringify(journal));
    } catch (e) {
    }
    return true;
  }
  function journalReste() {
    const journal = lireJournal();
    return Object.entries(journal).map(([fragment, n]) => ({ fragment, n })).sort((a, b) => b.n - a.n || a.fragment.localeCompare(b.fragment));
  }
  function viderJournalReste() {
    try {
      root.localStorage.removeItem(CLE_JOURNAL);
    } catch (e) {
    }
  }
