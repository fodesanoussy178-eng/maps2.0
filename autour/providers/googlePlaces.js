(function (root) {
  ...
  const typesVersCategorie = Object.freeze({
    restaurant:"resto", pizza_restaurant:"resto", meal_delivery:"resto",
  ...
  function texteDescription(place) {
    ...
    return (generative.overview && generative.overview.text) ||
      (generative.description && generative.description.text) ||
      (place && place.editorialSummary && place.editorialSummary.text) || "";
  }
  function attribution(photo) {
    return (photo && photo.authorAttributions || []).map((author) => ({
      name:author.displayName || "", url:author.uri || "",
    })).filter((author) => author.name);
  }
  function photoUrl(photo, apiKey) {
    if (!photo || !photo.name || !apiKey) return "";
    return "https://places.googleapis.com/v1/" + photo.name +
      "/media?maxHeightPx=640&skipHttpRedirect=false&key=" + encodeURIComponent(apiKey);
  }
  function normaliserPlace(place, config) {
    ...
    const photos = (p.photos || []).map((photo) => ({
      url: photoUrl(photo, config.apiKey), attribution: attribution(photo), source:"google_places",
    ...
    const resultat = {
      ...
      phone:p.nationalPhoneNumber || "", website:p.websiteUri || "", description:texteDescription(p),
      address:p.formattedAddress || "", source:"google_places", openNow:p.currentOpeningHours && p.currentOpeningHours.openNow,
  ...
  async function nearby(lat, lng, config, options) {
    ...
    const json = await request("places:searchNearby", config, {method:"POST", signal:options && options.signal,
      headers:{"Content-Type":"application/json", "X-Goog-Api-Key":config.apiKey, "X-Goog-FieldMask":champs}, body:JSON.stringify(body)});
  ...
  async function search(query, lat, lng, config, options) {
    const json = await request("places:searchText", config, {method:"POST", signal:options && options.signal,
      headers:{"Content-Type":"application/json", "X-Goog-Api-Key":config.apiKey, "X-Goog-FieldMask":champs},
      body:JSON.stringify({textQuery:query, languageCode:"fr", maxResultCount:12,
        locationBias:{circle:{center:{latitude:lat, longitude:lng}, radius:6000}}})});
    return (json.places || []).map((place) => normaliserPlace(place, config)).filter(Boolean);
  }
  async function details(placeId, config) {
    if (!placeId || !config || !config.apiKey) return null;
    const json = await request("places/" + encodeURIComponent(placeId) + "?languageCode=fr", config,
