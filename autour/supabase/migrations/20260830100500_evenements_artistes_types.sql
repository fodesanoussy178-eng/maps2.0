/* Champs canoniques partagés par les synchroniseurs et les RPC publiques. */
alter table public.events
  add column if not exists artist_names text[] not null default '{}',
...
begin
  if not exists (
    ...
    alter table public.events add constraint events_event_kind_check check (
      event_kind is null or event_kind = any(array[
        'concert','showcase','dj_set','festival','open_air','fete','fete_populaire',
        'fete_foraine','carnaval','kermesse','guinguette','bal','feu_artifice',
        'braderie','brocante','vide_grenier','marche_de_noel','fete_de_la_musique','fan_zone'
      ])
    );
  end if;
end
...
create index if not exists events_artist_names_gin_idx on public.events using gin (artist_names);
create index if not exists events_music_genres_gin_idx on public.events using gin (music_genres);
create index if not exists events_event_kind_idx on public.events (event_kind);

/* Maintenant : même emprise et même statut, avec les métadonnées canoniques. */
...
begin
  ...
  execute format($ddl$
    ...
    returns table (
      ...
      artist_names text[], music_genres text[], event_kind text,
    ...
    as $body$
      ...
      select e.id,e.publication_id,e.title,e.description,e.category,e.start_at,e.end_at,e.timezone,e.statut,
        ...
        e.artist_names,e.music_genres,e.event_kind,
...
/* Pour toi : conserver les tags existants et leur ajouter les champs canoniques. */
...
create function public.evenements_bassin(
  ...
  announcement_tags text[], artist_names text[], music_genres text[], event_kind text,
  importance_level text, importance_score integer, performers text[], organizer text,
  ticket_url text, announcement_provenance jsonb,
  metro_area text, territory_slug text, territory_distance_km double precision
)
...
as $function$
    ...
    select e.*, basin.group_slug, basin.slug as territory_slug, basin.distance_km,
      case
        ...
        when e.start_at <= p.instant and e.end_at >= p.instant
          then case when coalesce(e.cancelled,false) then 'past' when e.date_confidence='exact' then 'now' else 'unknown_date' end
  ...
  select c.id, c.publication_id, c.title, c.description, c.category, c.start_at, c.end_at,
    ...
    c.artist_names, c.music_genres, c.event_kind,
    c.importance_level, c.importance_score, c.performers, c.organizer, c.ticket_url,
    c.announcement_provenance, c.group_slug, c.territory_slug, c.distance_km
  from candidats c
  where c.statut <> 'past'
...
/* Le RPC annexe est également public : son contrat ne doit pas perdre les
   métadonnées si un client l'utilise pour les nouvelles annonces. */
...
returns table (
  ...
  importance_level text,importance_score integer,performers text[],organizer text,ticket_url text,announcement_provenance jsonb,
  metro_area text,territory_slug text,territory_distance_km double precision
)
language sql stable security invoker set search_path = ''
as $$
  select e.id,e.title,e.description,e.category,e.start_at,e.end_at,e.timezone,e.date_confidence,e.place_name,e.address,e.city,e.insee_code,
    ...
    e.tickets_open_at,e.announcement_tags,e.artist_names,e.music_genres,e.event_kind,e.importance_level,e.importance_score,e.performers,
    e.organizer,e.ticket_url,e.announcement_provenance,basin.group_slug,basin.slug,basin.distance_km
