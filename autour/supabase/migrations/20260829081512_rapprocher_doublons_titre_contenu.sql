-- Le premier jet a défait deux rapprochements justes que l'ancienne
-- déduplication avait posés à la main :
--
--   « Festival International du Court-Métrage »
--   « Festival International du Court Métrage de Lille »
--
--   « Spectacle : chanter pour résister »
--   « Spectacle musical « Chanter pour résister » »
--
-- Deux sources décrivent le même soir et l'une nomme plus long que l'autre.
-- L'égalité stricte des noyaux ne pouvait pas les voir. On ajoute donc
-- l'INCLUSION : un noyau contenu dans l'autre, à condition qu'il soit assez
-- long pour être une preuve — douze caractères — et que le lieu et l'heure
-- concordent déjà. « visite libre » ou « atelier » ne franchissent pas ce
-- seuil, et c'est exactement le but.
...
begin
  ...
  paires as (
    ...
    from candidats a join candidats b
      on a.jour = b.jour and a.id < b.id
     and (a.noyau = b.noyau
          or (length(a.noyau) >= 12 and position(a.noyau in b.noyau) > 0)
          or (length(b.noyau) >= 12 and position(b.noyau in a.noyau) > 0))
    where ST_DWithin(
