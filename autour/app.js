...
async function rechercheGeographique(q, signal){
  ...
  chargerZone(centre[0], centre[1], {reglages: reglagesZone(rechercheGeo)});
  majBoutons();
  return true;
}
...
/* Événements locaux publiés sur les agendas officiels OpenAgenda. La clé est
   une clé publique de lecture ; aucun événement n'est inventé en repli. */
const CLE_OPENAGENDA = "oa_pk_iWmzFaEujSWtgBdvWCdXQnRtgtrqkpokQbkRGVUrscALWleiwQesAucPIQNUGLjS";

function catOpenAgenda(evt){
  const mots = sansAccents((evt.title&&evt.title.fr||"")+" "+((evt.keywords&&evt.keywords.fr)||[]).join(" "));
  if(/screening|movie|film|projection|seance cinema/.test(mots)) return "event";
  ...
  if(/concert|scene|musique|spectacle/.test(mots)) return "concert";
...
/* ---- OpenAgenda : trois sauts, aucun filet ------------------------------

   Ce qui n'allait PAS, et ce qui allait bien. Les huit lectures d'agenda
   étaient déjà parallèles — ce n'était pas un 1+N séquentiel. Les vrais
   défauts étaient ailleurs, et ils sont tous du même genre : rien ne borne
   l'attente, et rien ne survit à un échec.

     · trois sauts en série avant le premier événement : nom de commune, puis
       liste d'agendas, puis les événements. Trois latences empilées ;
     · AUCUN délai maximum. Les autres fournisseurs coupent à quelques
       secondes ; ici, une requête qui pend faisait attendre `allSettled`
       indéfiniment, et le créneau restait en chargement ;
     · aucun cache : revenir sur une zone déjà vue relançait les dix requêtes ;
     · aucun repli. Un échec rendait `null`, et les événements déjà affichés
       disparaissaient — un vide provoqué par une panne, exactement ce qu'on
       s'interdit ailleurs.

   Corrigé sans changer ce que la fonction rend : un délai par requête, trois
   requêtes simultanées au plus (huit d'un coup sur le même hôte invitent le
   rate limit), un cache de zone, et la dernière réponse valide conservée. */
const DELAI_OPENAGENDA_MS = 6000;
const CONCURRENCE_OPENAGENDA = 3;
const CACHE_OPENAGENDA_MS = 10 * 60 * 1000;
const cacheOpenAgenda = new Map();

function cleOpenAgenda(lat,lng){ return lat.toFixed(2)+","+lng.toFixed(2); }

async function fetchOpenAgenda(url){
  const arret = AbortSignal.timeout ? AbortSignal.timeout(DELAI_OPENAGENDA_MS) : undefined;
  return fetch(url, {signal:arret});
}

/* Exécute au plus `limite` tâches à la fois. Une tâche qui échoue n'arrête
   pas les autres : c'est le contrat de `allSettled`, conservé ici. */
async function parLots(taches, limite){
  const resultats = new Array(taches.length);
  let curseur = 0;
  const ouvriers = Array.from({length:Math.min(limite, taches.length)}, async ()=>{
    while(curseur < taches.length){
      const i = curseur++;
      try{ resultats[i] = {status:"fulfilled", value:await taches[i]()}; }
      catch(e){ resultats[i] = {status:"rejected", reason:e}; }
    }
  });
...
async function evenementsOpenAgenda(lat,lng){
  const cle = cleOpenAgenda(lat,lng);
  const enCache = cacheOpenAgenda.get(cle);
  // stale-while-revalidate : ce qu'on a déjà vu repart tout de suite
  if(enCache && Date.now() - enCache.le < CACHE_OPENAGENDA_MS) return enCache.items;
  const frais = await evenementsOpenAgendaReseau(lat,lng);
  if(Array.isArray(frais)){
    cacheOpenAgenda.set(cle, {items:frais, le:Date.now()});
    return frais;
  }
  /* Échec : on rend la dernière réponse valide plutôt que rien. Des données
     d'il y a un quart d'heure valent mieux qu'un écran qui se vide, et le
     créneau ne saute pas. */
  return enCache ? enCache.items : null;
}

async function evenementsOpenAgendaReseau(lat,lng){
  if(!CLE_OPENAGENDA) return null;
  try{
    // La lecture transverse /v2/events est expérimentale et n'est pas ouverte
    // à cette clé. On découvre donc les agendas officiels de la commune, puis
    // on lit leurs événements publiés avec les routes publiques documentées.
    const ville = await nomCommune(lat,lng);
    const rechercheAgendas = new URLSearchParams({
      key:CLE_OPENAGENDA, search:ville, size:"8", official:"1"
    });
    PERF.requete("openagenda");
    const rAgendas = await fetchOpenAgenda("https://api.openagenda.com/v2/agendas?"+rechercheAgendas.toString());
    if(!rAgendas.ok){ console.error("OpenAgenda agendas : réponse HTTP", rAgendas.status); return null; }
    const agendas = ((await rAgendas.json()).agendas||[]).slice(0,8);
    const segments = await parLots(agendas.map(agenda=>async ()=>{
      const params = new URLSearchParams({
        key:CLE_OPENAGENDA, size:"30", detailed:"1"
      });
      params.append("relative[]","current");
      params.append("relative[]","upcoming");
      PERF.requete("openagenda");
      const r = await fetchOpenAgenda("https://api.openagenda.com/v2/agendas/"+agenda.uid+"/events?"+params.toString());
      if(!r.ok) throw new Error("agenda "+agenda.uid+" : HTTP "+r.status);
      return (await r.json()).events || [];
    }), CONCURRENCE_OPENAGENDA);
    const uniques = new Map();
    segments.forEach(segment=>{
      if(segment.status !== "fulfilled") return;
      segment.value.forEach(evt=>uniques.set(evt.uid,evt));
    });
    const bruts = [...uniques.values()];
    journal.info("OpenAgenda:", bruts.length, "événement(s) bruts depuis", agendas.length, "agenda(s)");
    /* Toutes les séances, pas seulement la première : une série hebdomadaire
       transmise par son seul `firstTiming` était datée sur une séance passée
       depuis des mois, et par sa seule période sur toute son étendue. Le
       moteur temporel choisit ensuite la prochaine occurrence réelle. */
    const occurrencesDe = (evt)=>{
      const brutes = Array.isArray(evt.timings) && evt.timings.length
        ? evt.timings
        : [evt.nextTiming, evt.firstTiming].filter(Boolean);
      return brutes.map(t=>{
        const debut = t && t.begin ? new Date(t.begin).getTime() : null;
        const fin = t && t.end ? new Date(t.end).getTime() : null;
        return debut == null && fin == null ? null : {start:debut, end:fin};
      }).filter(Boolean).sort((a,b)=>(a.start||0)-(b.start||0));
    };
    // celle qui compte à cet instant : en cours, sinon la prochaine à venir
    const occurrenceUtile = (occ)=>{
      if(!occ.length) return null;
      const t = Date.now();
      return occ.find(o=>o.start != null && o.start <= t && (o.end == null || o.end > t))
        || occ.find(o=>o.start != null && o.start > t)
        || occ[occ.length-1];
    };

    const classes = bruts.map(evt=>{
      const loc = evt.location || {};
      if(loc.latitude==null || loc.longitude==null) return null;
      if(distanceM(lat,lng,Number(loc.latitude),Number(loc.longitude)) > 8000) return null;
      const occurrences = occurrencesDe(evt);
      const seance = occurrenceUtile(occurrences);
      return normaliserItem({
        occurrences,
        id:"oa"+evt.uid, cat:catOpenAgenda(evt),
        titre:(evt.title && (evt.title.fr||Object.values(evt.title)[0])) || "Événement",
        description:(evt.description && (evt.description.fr||Object.values(evt.description)[0])) || "",
        keywords:(evt.keywords && (evt.keywords.fr||Object.values(evt.keywords)[0])) || [],
        eventCategory:(evt.keywords && (evt.keywords.fr||[]).join(" ")) || "",
        adresse: loc.name || loc.address || commune,
        cp: [loc.postalCode, loc.city].filter(Boolean).join(" ") || commune,
        quand: seance && seance.start != null
          ? new Date(seance.start).toLocaleString("fr-FR",
              {weekday:"short",day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"})
          : "Voir sur place",
        debutLe: seance ? seance.start : null,
        // fin réelle, pour ne plus afficher un événement d'hier ; sans fin
        // publiée on suppose trois heures plutôt que rien
        finLe: seance
          ? (seance.end != null ? seance.end
             : (seance.start != null ? seance.start + 3*3600*1000 : null))
          : null,
        gratuit: !(evt.registration && evt.registration.price), prix:0,
        places:null, qr:false, par:"OpenAgenda",
        isTemporary:true,
        url:evt.canonicalUrl || evt.originAgenda && evt.originAgenda.url || "",
        image:evt.image ? (evt.image.base || "")+(evt.image.filename || "") : "",
        lat:loc.latitude, lng:loc.longitude
      }, "openagenda");
    }).filter(Boolean);
    journaliserPipeline("OpenAgenda", bruts.length, classes, classes);
    return classes;
  }catch(e){ console.error("OpenAgenda: échec de la requête", e); return null; }
}
...
function chargerZone(lat, lng, opts){
  const o = opts || {};
  if(!o.sansCarte && (!map || map.getZoom() < ZOOM_MIN_CHARGEMENT)) return Promise.resolve([]);
...
async function segmentTrajet(profil, depart, arrivee){
  ...
  if(reel) return { coords:reel.coords, reel:true, min:reel.min };
  const d = distanceM(depart[0],depart[1],arrivee[0],arrivee[1]);
  return { coords:[depart,arrivee], reel:false, min: tempsTrajetMinutes(d, VITESSES_KMH[profil]||15) };
}
...
function rendreOptions(options, iActif){
  return options.map((o,i)=>
    '<button class="'+(i===iActif?'conseille':'')+'" data-opt="'+i+'">'+
      EMOJI_MODE[o.mode]+' '+LABEL_MODE[o.mode]+' · '+o.min+' min</button>'
  ).join("");
}
...
function liensItinerairesExternes(depart, destination){
  const lien = (label, fournisseur, mode)=>
    '<a class="trajet-lien" target="_blank" rel="noopener" data-provider="'+fournisseur+'" href="'+
      esc(urlItineraireExterne(fournisseur, mode, depart, destination))+'">'+label+'</a>';
  return '<div class="trajet-externes">'+
    '<section class="trajet-externe"><strong>🚗 Voiture</strong>'+
      '<span>Ouvrir l’itinéraire</span><div class="trajet-liens">'+
        lien("Google Maps", "google", "driving")+
        lien("Apple Plans", "apple", "driving")+
        lien("Waze", "waze", "driving")+
      '</div></section>'+
    '<section class="trajet-externe"><strong>🚌 Transports</strong>'+
      '<span>Voir en transports</span><div class="trajet-liens">'+
...
async function afficherTrajet(l){
  ...
  const droit = (profil)=>({ coords:[depart,dest], reel:false,
    min:tempsTrajetMinutes(dVol, VITESSES_KMH[profil]) });
  const options = [
    {mode:"pied", min:droit("pied").min, segments:[droit("pied")]},
    {mode:"velo", min:droit("velo").min, segments:[droit("velo")]},
  ];
  let iActif = 0;

  panneau.hidden = false;
  panneau.innerHTML =
    '<p class="trajet-interne-titre">Itinéraires dans Autour</p>'+
    '<div class="trajet-temps" id="trajetTemps"></div>'+
    '<p class="trajet-aide" id="trajetAide">Calcul des parcours à pied et à vélo…</p>'+
    liensItinerairesExternes(depart, dest);

  const peindre = ()=>{
    if(numeroDemande !== afficherTrajet.numeroDemande) return false;
    const zone = $("#trajetTemps");
    if(!zone) return false;
    zone.innerHTML = rendreOptions(options, iActif);
    zone.querySelectorAll("[data-opt]").forEach(b=>b.onclick=()=>{
      iActif = Number(b.dataset.opt);
      peindre();
      entrerNav(options[iActif], l.titre);
    });
    dessinerSegments(options[iActif].segments);
    return true;
  };
  peindre();
  ...
  Promise.all([
    ...
    options[0] = {mode:"pied", min:pied.min, segments:[pied]};
    options[1] = {mode:"velo", min:velo.min, segments:[velo]};
    if(!peindre()) return;
    const aide = $("#trajetAide");
    if(aide) aide.textContent = (pied.reel || velo.reel)
      ? "Touche À pied ou Vélo pour ouvrir le parcours dans Autour."
      : "Routage indisponible : les temps sont estimés à vol d’oiseau.";
  });
}
